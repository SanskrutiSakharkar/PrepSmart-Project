...
function CodingRound() {
  ...
  const handleAIQuestion = async () => {
    ...
    try {
      ...
      setOutput("Failed to generate AI question.");
    }
  };

  // Save AI-generated question to DB
  const handleSaveAIQuestion = async () => {
    if (!selected) return;
    setSaving(true);
    try {
      await axios.post("/api/coding-round/save-ai-question",
        { question: selected },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <div className="codinground-container">
        ...
        <div className="codinground-toolbar">
          {Object.keys(sectionLabels).map(sec => (
            <button
              key={sec}
              className={`codinground-btn${section === sec ? " active" : ""}`}
              onClick={() => setSection(sec)}
            >{sectionLabels[sec]}</button>
          ))}

          <label className="codinground-label">
            Theme:&nbsp;
            <select
              className="codinground-select"
              value={theme}
          ...
          <button className="codinground-btn codinground-format" onClick={formatCode}>
            Format Code
          </button>

          <button className="codinground-btn codinground-generate" onClick={handleAIQuestion}>
            + Generate AI Question
          </button>

          {(selected && !selected._id) && (
            <button
              className="codinground-btn codinground-save"
              onClick={handleSaveAIQuestion}
              disabled={saving}
            >
              {saving ? "Saving..." : "Save AI Question"}
            </button>
          )}
        </div>
        ...
        <div className="codinground-questionselect">
          ...
          <select
            className="codinground-select"
            value={selected?._id || selected?.title || ""}
            onChange={e => {
          ...
          >
            {questions.map(q => (
              <option key={q._id || q.title} value={q._id || q.title}>
                {q.title}
              </option>
            ))}
          </select>
        ...
        <div className="codinground-desc">
        ...
        <form onSubmit={handleSubmit}>
          ...
          />
          <button type="submit" className="codinground-btn codinground-submit">
            Submit
          </button>
        </form>

        {/* Output */}
        <div className="codinground-output">
          <h4>Output:</h4>
          <pre className="codinground-pre">{output}</pre>
        </div>

        {/* Personalized Suggestions as a Card */}
        {suggestions && suggestions.length > 0 && (
          <div className="codinground-suggestions-card">
            <h4>Personalized Suggestions:</h4>
            <ul className="codinground-suggestions-list">
