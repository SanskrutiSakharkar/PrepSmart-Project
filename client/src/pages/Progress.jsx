...
const CARD_HELP = {
  coding: "Your accuracy in PrepSmart coding rounds",
  tech: "Accuracy answering tech MCQs",
  sentiment: "Positivity detected in voice answers",
...
export default function Progress() {
  ...
  return (
    <div className="progress-outer">
      {/* BACK BUTTON */}
      <button className="progress-back-btn" onClick={() => navigate("/dashboard")}>
      ...
      <div className="progress-dashboard">
        ...
        <div className="progress-subtitle">
          Track your AI interview prep progress. <br />
          <span style={{ fontSize: "1rem", color: "#666" }}>
            See how you’re improving and get actionable AI feedback.
          </span>
        </div>
          ...
          {["coding", "tech", "sentiment", "filler", "resume"].map(key => (
            <div className="growth-card" key={key}>
              ...
              <h3>{{
                ...
                tech: "Tech Accuracy",
                sentiment: "Sentiment",
                filler: "Filler Words (↓ is better)",
                resume: "Resume Match %",
              }[key]}</h3>
              <div className="card-value">
                {showRange(
                  data.growth[key],
                  key === "filler" ? "filler_count" :
                  key === "resume" ? "match_percent" :
                  key === "sentiment" ? "sentiment" : "accuracy",
                  key === "sentiment" ? 2 : 1
                )}
                {key === "resume" || key === "coding" || key === "tech" ? "%" : ""}
              </div>
              <span className={data.growth[key].diff >= 0 || key === "filler" ? "up" : "down"}>
                {data.growth[key].diff >= 0 || key === "filler" ? "▲" : "▼"} {Math.abs(data.growth[key].diff).toFixed(key === "sentiment" ? 2 : 1)}
                {key === "resume" || key === "coding" || key === "tech" ? "%" : ""}
              </span>
        ...
        <div className="chart-section">
          ...
          <div className="chart-sub">AI analysis of how positive you sound in voice answers.</div>
        ...
        <div className="chart-section">
          ...
          <div className="chart-sub">Filler words like "um", "uh", "like". Lower is better.</div>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="chart-section">
          ...
          <div className="chart-sub">How closely your resume matches jobs you applied to.</div>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="feedback-log">
          ...
          <div className="feedback-intro">
            Get actionable tips after each practice, review and apply to boost your next round!
          </div>
          <div className="feedback-list">
            {data.feedbackLog && data.feedbackLog.length > 0 ? (
              data.feedbackLog.map((f, i) => (
                <div className="feedback-entry" key={i}>
                  <span className="feedback-date">
                    {f.date
                      ? new Date(f.date).toLocaleDateString("en-NZ", {
                          day: "numeric",
                          month: "short",
                          year: "numeric",
                        })
              ...
              <div className="feedback-none">
                No feedback available yet. Try submitting a round to get suggestions!
              </div>
            )}
          </div>
        </div>
