...
// === MAPPING BETWEEN UI SECTIONS AND DB TOPICS ===
const SECTIONS = ["React", "Python", "SQL"];
const SECTION_TO_TOPIC = {
  React: "React",
  Python: "Python",
  SQL: "SQL",      // Change this to "Databases" if your DB still uses that!
};

function AnswerTechnicalQuestions() {
  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `/api/tech-questions?topic=${encodeURIComponent(topic)}`,
        token
      ...
      setApiError(
        ...
        err.message ||
        "Error loading questions. Please login again."
      );
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      console.error("API error generating AI question:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleNext = () => {
    ...
    setCurrentIdx((idx) => (idx + 1) % questionList.length);
    setAnswer("");
    setFeedback("");
  };
  const handlePrev = () => {
    if (questionList.length === 0) return;
    setCurrentIdx((idx) => (idx - 1 + questionList.length) % questionList.length);
    setAnswer("");
    setFeedback("");
  };
  ...
  const handleGetFeedback = async (question, ans) => {
    ...
    try {
      ...
      setFeedback("");
      console.error("API error getting feedback:", err);
    }
  };
      ...
      <div className="answertech-container">
        ...
        <div className="answertech-sections">
          {SECTIONS.map(sec => (
            <button
              key={sec}
              className={`answertech-tab${selectedSection === sec ? " active" : ""}`}
        ...
        {apiError && (
          <div className="answertech-error">
            {apiError}
          </div>
        )}
        ...
        <div className="answertech-card">
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button className="answertech-btn" onClick={handleGenerateAI} disabled={loading}>
          ...
          {aiQuestion && (
            <div style={{ marginTop: 18 }}>
              <textarea
                className="answertech-textarea"
                placeholder="Write your answer here..."
                value={answer}
                onChange={e => setAnswer(e.target.value)}
              />
              <button
                className="answertech-btn"
                style={{ marginTop: 10 }}
                onClick={() => handleGetFeedback(aiQuestion, answer)}
              >
                Get Feedback
              </button>
              {feedback && (
                <div className="answertech-feedback">
        ...
        {!aiQuestion && (
          <div className="answertech-card">
            ...
            {loading ? (
              ...
              <div style={{ color: "#aaa" }}>No questions saved yet. Generate or add some!</div>
            ) : (
              <>
                <div style={{ fontWeight: 600, fontSize: 16, minHeight: 36 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}>
                  ...
                  <button className="answertech-btn" onClick={() => handleGetFeedback(questionList[currentIdx].question, answer)}>
                    Get Feedback
                  </button>
...
export default AnswerTechnicalQuestions;
