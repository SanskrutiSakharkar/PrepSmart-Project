...
router.get("/summary", authenticate, async (req, res) => {
  try {
    ...
    const sentimentTrend = feedbackVoice.map(f => ({ date: f.createdAt, sentiment: typeof f.sentiment === "number" ? f.sentiment : 0 }));
    const fillerTrend = feedbackVoice.map(f => ({ date: f.createdAt, filler_count: typeof f.fillerWords === "number" ? f.fillerWords : 0 }));
    const codingTrend = coding.map(c => ({ date: c.submittedAt, correct: c.passed ? 1 : 0, total: 1, accuracy: c.passed ? 100 : 0 }));
    const resumeTrend = analysis.map(a => ({ date: a.analyzedAt, match_percent: typeof a.matchScore === "number" ? a.matchScore : 0 }));
    ...
    const voiceFeedbacks = voice.map(v => ({
      date: v.timestamp,
      feedback: Array.isArray(v.suggestions) && v.suggestions.length > 0 ? v.suggestions.join("; ") : v.feedback || "(No feedback)",
      type: "Voice"
    }));

    const codingFeedbacks = coding.filter(c => Array.isArray(c.suggestions) && c.suggestions.length > 0).map(c => ({
      date: c.submittedAt,
    ...
    const feedbackResults = feedback.map(f => ({
      date: f.createdAt,
      feedback: Array.isArray(f.suggestions) && f.suggestions.length > 0 ? f.suggestions.join("; ") : (f.matchScore !== undefined ? `Resume match: ${f.matchScore}%` : f.sentiment || f.emotion || "(No feedback)"),
    ...
    const allFeedbacks = [...voiceFeedbacks, ...codingFeedbacks, ...feedbackResults, ...resumeFeedbacks]
